...
function calculateConversions(changedField, fields) {
  ...
  if (changedField === 'stone' || changedField === 'pound') {
    const kg = poundsToKg(stone * 14 + pound)
    return { kg }
  }
  if (changedField === 'metres') {
    const feetDec = metresToYards(metres) * 3
    const feet = Math.floor(feetDec)
    const inches = (feetDec - feet) * 12
    return { feet, inches }
  }
...
export { calculateConversions as default }